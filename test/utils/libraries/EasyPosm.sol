...
library EasyPosm {
    ...
    function mint(
        ...
        return _mintInternal(posm, input);
    }
    ...
    function _mintInternal(IPositionManager posm, MintInput memory input)
        private
        returns (uint256 tokenId, BalanceDelta delta)
    {
        (Currency currency0, Currency currency1) = (input.poolKey.currency0, input.poolKey.currency1);
        ...
        mintData.params[0] = _encodeMintPositionParams(input);
        mintData.params[1] = abi.encode(currency0, currency1);
        mintData.params[2] = abi.encode(currency0, input.recipient);
        mintData.params[3] = abi.encode(currency1, input.recipient);

        tokenId = posm.nextTokenId();
        uint256 valueToPass = currency0.isAddressZero() ? input.amount0Max : 0;
        posm.modifyLiquidities{value: valueToPass}(abi.encode(mintData.actions, mintData.params), input.deadline);
    ...
    function _encodeMintPositionParams(MintInput memory input) private pure returns (bytes memory) {
        return abi.encode(
            input.poolKey,
            input.tickLower,
            input.tickUpper,
